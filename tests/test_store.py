...
def test_search_returns_matching_entries(tmp_store_dir: Path) -> None:
    store = MemoryStore(tmp_store_dir)
    store.write(MemoryEntry("Prefers Python over JavaScript", "preferences", "claude", "2026-03-03T10:00:00"))
...
def test_forget_removes_matching_entries(tmp_store_dir: Path) -> None:
    store = MemoryStore(tmp_store_dir)
    store.write(MemoryEntry("Prefers Python", "preferences", "claude", "2026-03-03T10:00:00"))
    store.write(MemoryEntry("Prefers dark mode", "preferences", "claude", "2026-03-03T10:01:00"))
